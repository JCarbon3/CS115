...
def FA(x,y,cin):
    ...
    sum = XOR3(x,y,cin)
    carry_out = gor(gor(gand(x,y),gand(x,cin)),gand(y,cin))
...
def test_fourBitAdd():
    ...
    seven = (0,1,1,1)
    fourteen = (1,1,1,0)
    c,ww = fourBitAdd(zero,five)
    assert(ww == (0,1,0,1) and c == 0)
    c,ww = fourBitAdd(five,seven)
    assert(ww ==(1,1,0,0) and c == 0)
    c,ww = fourBitAdd(five,fourteen)
    assert(ww == (0,0,1,1) and c == 1)
    c,ww = fourBitAdd(seven,fourteen)
    assert(ww == (0,1,0,1) and c == 1)
